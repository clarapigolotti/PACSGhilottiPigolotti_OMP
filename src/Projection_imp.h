...
template<UInt ORDER>
void Projection<ORDER,2,3>::computeProjection(DEData& data)
{
  Point res;

  // (1): nearest node to each point
  std::vector<UInt> nearest_node;
  nearest_node.reserve(data.getNumberofData());
  for(UInt d=0; d<data.getNumberofData(); d++){
    std::pair<Real, UInt> min_dist (computeDistance(data.getDatum(d), mesh_.getPoint(0)), 0);
    Real dist;
    for(UInt n=1; n<mesh_.num_nodes(); n++){
      dist = computeDistance(data.getDatum(d), mesh_.getPoint(n));
  ...
  std::vector<std::vector<UInt>> patch_element; // to each node -> id of elements that contains that node
  patch_element.reserve(data.getNumberofData());
  for(UInt i=0; i<data.getNumberofData(); i++){
    patch_element.push_back(computeNodePatch(nearest_node[i]));
  }

  // (3): project each point on the patch of the nearest node
  for(UInt i=0; i<data.getNumberofData(); i++){
    ...
    for(auto elem: patch_element[i]){
      ...
      std::pair<Point, Real> proj = project(element, data.getDatum(i));
